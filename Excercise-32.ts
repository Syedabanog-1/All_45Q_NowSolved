...
• Make sure your comparison is case insensitive. If 'John' has been
 ...
 let current_Users: string [] = ['Tayyaba','Fatima','Malaika','Ayma','Shams','Pakiza','tahira'];
 let new_Users: string [] = ['Chandni','tayyaba','sunaira','Ayma','malaika','farah','roshni']


new_Users.forEach(newUsers => { 
  let  new_UsersL = newUsers.toLowerCase();
  //some methode to match current and new users
  let currentUsersL = current_Users.some(currentUsers => currentUsers.toLowerCase() === new_UsersL.toLowerCase()) //true/false
  // apply condition
  if (new_UsersL){

    console.log( ` ${currentUsersL}:  ${newUsers} `);
  } else {
    console.log(` ${currentUsersL}: ${newUsers}  `);
